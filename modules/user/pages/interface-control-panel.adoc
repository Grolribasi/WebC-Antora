...
Открыть xref:interface-user-menu.adoc[_меню пользователя_]. На кнопке отображается фото (xref:employeesEmployee.adoc#addPhoto[если установлено]), а также цветовая метка, соответствующая xref:employeesEmployeeFields.adoc#employeeStatuses[состоянию] учётной записи сотрудника.
+
[NOTE]
====

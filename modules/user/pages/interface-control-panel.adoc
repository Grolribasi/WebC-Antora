...
Открыть xref:interface-user-menu.adoc[меню пользователя]. На кнопке отображается фото (xref:directories/staff/employee.adoc#photo[если установлено]), а также цветовая метка, соответствующая xref:directories/staff/employee-fields.adoc#statuses[состоянию] учётной записи сотрудника.
+
[NOTE]
====

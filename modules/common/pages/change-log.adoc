...
Добавлена возможность предварительного просмотра xref:admin:preview-r7.adoc[с помощью "Р7-Офис. Сервер документов"]. "Р7-Офис. Сервер документов" расширяет стандартную функциональность, позволяя отображать документ напрямую и копировать данные непосредственно из файла.
...
Теперь при изменении настроек папок счётчик непрочитанных карточек можно обновить вручную или он обновится автоматически через 10 минут. См. подробнее xref:admin:cache-folders.adoc[].

...
|ExcludeFromIL |Задает или возвращает флаг, указывающий, будет ли данное свойство исключено из пакета свойств элемента управления, передаваемых в клиентскую часть {wc}а. Свойство с установленным флагом ExcludeFromIL остается настраиваемым в Конструкторе web-разметок.

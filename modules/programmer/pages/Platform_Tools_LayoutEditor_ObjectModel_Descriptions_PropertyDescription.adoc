...
|ExcludeFromIL |Задает или возвращает флаг, указывающий, будет ли данное свойство исключено из пакета свойств элемента управления, передаваемых в клиентскую часть Web-клиента. Свойство с установленным флагом ExcludeFromIL остается настраиваемым в Конструкторе web-разметок. 
...
private PropertyDescription GetSliderPropertyDescription()
{
 ...
 return new PropertyDescription
 {
  ...
  ExcludeFromIL = true, // Свойство не должно передаваться на клиентскую сторону Web-клиента

...
. Файл `SuperControl.xml` копируем в папку `C:\Program Files (x86)\{dv}\WebClient\5.5\Plugins\`.
...
. Полученный файл `Site\Content\Modules\...\extension.js` копируем в папку `C:\Program Files (x86)\{dv}\WebClient\5.5\Site\Content\Modules\SuperControlExtension\` на сервере {wc}а.

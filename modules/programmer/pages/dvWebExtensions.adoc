...
. Сохраните разметку.
. Установите или обновите программу _DVWebTool_. Cм. пункт xref:user:prepareInstallAdditionalComponents.adoc#dvWebTool[Установка и запуск программы DVWebTool] руководства пользователя {wc}.

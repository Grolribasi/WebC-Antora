...
Описание ссылки формируется с помощью метода, зарегистрированного в сервисе ссылок link:BackOffice_WebClient_Links_ILinksService.md[ILinksService] и указанного в качестве «Метода для описания» в настройках элемента «Ссылки» в Конструкторе Web-разметок.
...
Разработанный метод необходимо зарегистрировать в серверном расширение Web-клиента:
...
----
   ...
   // В первом параметре нужно указать любое текстовое название метода (без пробелов, спецсимволов и пр.), по которому его можно вызывать. Данное название метода нужно будет указать в конструкторе Web-разметок
...
. Откройте в Конструкторе Web-разметок разметку карточки, в которой описание ссылок должно формироваться с помощью расширения.

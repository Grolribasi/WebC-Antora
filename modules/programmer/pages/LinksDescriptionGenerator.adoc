...
Описание ссылки формируется с помощью метода, зарегистрированного в сервисе ссылок link:BackOffice_WebClient_Links_ILinksService.adoc[ILinksService] и указанного в качестве "Метода для описания" в настройках элемента "Ссылки" в программе {kvr}.
...
. link:ServerExtensionNew.adoc[Создайте проект серверного расширения]

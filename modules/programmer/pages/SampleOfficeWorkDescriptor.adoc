...
Для описание данного элемента управления сформирует link:CreateTextControlDescriptor.adoc[текстовый описатель] со следующим содержимым:
...
Редактор RefCasesFieldMetadataEditor является стандартным редактором link:DesignerExtensionWithFieldEditor.adoc[редактором для выбора поля карточки], реализованном в классе `{dv}.BackOffice.WebLayoutsDesigner.Editors.FieldMetadataEditor`.

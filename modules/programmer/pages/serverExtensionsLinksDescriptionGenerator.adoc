...
Описание ссылки формируется с помощью метода, зарегистрированного в сервисе ссылок xref:BackOffice_WebClient_Links_ILinksService.adoc[ILinksService] и указанного в качестве *Метода для описания* в настройках элемента `_Ссылки_` в программе {kvr}.

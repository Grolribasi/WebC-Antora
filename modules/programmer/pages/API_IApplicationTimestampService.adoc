...
*Пространство имен*: `DocsVision.WebClient.Services`
...
Реализация метода UpdateApplicationTimeStamp используется при вызове веб-метода `GET http://{Адрес Web-клиента}/WebClient/Navigator/ResetClientCache`. Если требуется, стандартная реализация сервиса (приведена далее) может быть переопределена собственной.

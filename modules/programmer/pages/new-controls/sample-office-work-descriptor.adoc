...
Для описание данного элемента управления сформируем xref:new-controls/create-text-descriptor.adoc[текстовый описатель] со следующим содержимым:
...
Редактор `RefCasesFieldMetadataEditor` является стандартным редактором xref:layout-designer-extensions/default-editor.adoc[редактором для выбора поля карточки], реализованном в классе `{dv}.BackOffice.WebLayoutsDesigner.Editors.FieldMetadataEditor`.

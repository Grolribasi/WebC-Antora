...
Пример реализации генератора изображений для получения изображения штампа ЭП приведён в пункте xref:createSignatureStampGenerator.adoc[Разработка генератора штампов электронных подписей].

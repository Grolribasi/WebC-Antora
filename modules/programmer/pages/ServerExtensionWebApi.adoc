...
. Создайте проект link:ServerExtensionNew.adoc[серверного расширения].
...
Пример клиентского расширения, использующего метод `Exists`, приведен в пункте link:ClientExtensionsServices.adoc[Разработка расширения с клиентским сервисом].